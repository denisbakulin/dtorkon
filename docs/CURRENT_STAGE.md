...
- приватная зона умеет:
  ...
  - управлять текущим audio через pinned player с click/drag seek по waveform, временем `прошло / всего`, позицией трека в плейлисте, скоростью воспроизведения и previous/next;
