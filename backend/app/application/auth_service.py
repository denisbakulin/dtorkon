...
class AuthService:
    ...
    async def login(
        ...
        *,
        ...
        if not hmac.compare_digest(payload.username, self.settings.admin_username):
            raise AppError(
                status_code=401,
        ...
        if not hmac.compare_digest(payload.password, self.settings.resolved_admin_password):
            raise AppError(
                status_code=401,
                code="invalid_credentials",
                message="Неверный логин или пароль",
        ...
        return session_to_response(session_record, self.settings.admin_username)
